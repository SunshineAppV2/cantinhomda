...
export class RequirementsService {
    ...
    findAll(query: { dbvClass?: any, specialtyId?: string, userId?: string, userClubId?: string }) {
        ...
        // If we have both a Universal and a Club requirement with the same Code/Class/Specialty,
        // show the CLUB one.
        if (query.userClubId) {
            ...
            // Index Club Reqs by "Key" (e.g. Class+Code or Specialty+Code)
            ...
            const finalUniversal = universalReqs.filter(u => {
                if (u.code) {
                    ...
                    // If Club has an override, hide this Universal one
                    return !clubMap.has(key);
                }
                return true; // No code, keep it (cannot safely deduce overlap)
            });
